...
def main(cfg):
    ...
    dirname = "_".join(["heun", REGION+str(num_processing_steps), "NN"+str(NN), datetime.datetime.now().isoformat()])
    logdir = os.path.join("log", dirname)
    logdir = logdir.replace('\\', '/')
    logdir = logdir.replace(':', '_')
    modeldir = os.path.join("model", dirname)
    modeldir = modeldir.replace('\\', '/')
    modeldir = modeldir.replace(':', '_')
...
#    edge_num_embeddings = torch.max(edge_attr).item() + 2
    edge_num_embeddings = 64 # USED IN PAPER
...
#         output_tensors, time_derivatives, spatial_derivatives, _ = model(input_graphs, sp_L, num_processing_steps, D, PDE)
        ...
        optimizer.zero_grad()
        loss.backward()
#        torch.nn.utils.clip_grad_norm_(model.parameters(), 1) # Prevent Gradient Blowup
        optimizer.step()
...
#        ## SAVE PLOT
#        if iter_ % 500 == 0:
#            xs = np.append(xs, np.array([iter_]))
#            ys1 = np.append(ys1, loss.item())
#            ys2 = np.append(ys2, np.array([val_losses_sup[-1]]))
#            #            print("iter = ", iter_, xs, ys1, ys2)
#            print("iter = ", iter_)
#            print("xs = ", xs)
#            print("ys1 (Training error) = ", ys1)
#            print("ys2 (Validation error) = ", ys2)

        ## SAVE PLOT
        if iter_ % 500 == 0:
            xs = np.append(xs, np.array([iter_]))
            ys1 = np.append(ys1, loss.item()/num_nodes)
            ys2 = np.append(ys2, np.array([val_losses_sup[-1]/num_nodes]))
            #            print("iter = ", iter_, xs, ys1, ys2)
            print("iter = ", iter_)
            print("xs = ", xs)
            print("ys1 (Training error) = ", ys1)
    ...
    logging.info("[Training]The smallest supervised loss: {:.4f}({:.4f}) at {}/{}"
                 .format(np.min(losses_sup), np.min(losses_sup)/num_nodes, np.argmin(losses_sup), len(losses_sup)))
    logging.info("[Vali]The smallest supervised loss: {:.4f}({:.4f}) at {}/{}"
                 .format(np.min(val_losses_sup), np.min(val_losses_sup)/num_nodes, np.argmin(val_losses_sup), len(val_losses_sup)))


    logging.getLogger('matplotlib.font_manager').setLevel(logging.ERROR)
    plt.plot(xs, ys1, label = 'training')
    plt.plot(xs, ys2, label = 'validation')
    plt.ylim([0,2])
    plt.legend()
    dirname = dirname.replace(':', '_')
    plt.savefig(f"./{dirname}.png", format='png')
    ...
    logging.info("[Test]MSE across all predictions: {:.4f}({:.4f})".format(np.mean(losses_te), np.mean(losses_te)/num_nodes))

    logging.info("MODE:{}\t{:.4f}({:.4f})\t{:.4f}({:.4f})".format(MODE, np.min(losses_sup), np.min(losses_sup)/num_nodes, 
                                                                  np.mean(losses_te), np.mean(losses_te)/num_nodes))
